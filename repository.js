window.addEventListener("load", function () {
    ...
    //modalPopupContent.appendChild(modalClose);
    ...
    chrome.storage.sync.get(null, function (items) {
        ...
        /** for testing.. wtf just happened */
       // console.log(items[allKeys[2]][2].highlightTextContent);

        console.log("allKeys length: " + allKeys.length);
        console.log("allKeys: " + allKeys);

        //let appendSitesUnder = document.getElementById("appendSitesUnder");
        ...
        for (let i = 0; i < allKeys.length; i++){
            ...
            if (hostnameArray.includes(urlObject.hostname)){
                ...
                // siteColumnUrlText.innerHTML = allKeys[i];
            ...
            /**
            siteColumn.onclick = function (this) {
                toggleDropdown(element.nextSibling.nextSibling);
            } */
            ...
            /** going to use this so I don't have to remake my highlight storage system (even though it would be faster/more efficient lmao */


            /** This used to append each highlight's text under the main siteColumnContainer. But I'm moving it to a popup_html instead */
            /**
            for (let k = 0; k < items[allKeys[i]].length; k++){
                let dropdownSitesUrl = document.createElement("div");
                dropdownSitesUrl.className = "dropdownSitesUrl";
                dropdownSitesUrl.innerHTML = items[allKeys[i]][k].highlightTextContent;
                dropdownSites.appendChild(dropdownSitesUrl);
            }
             */
...
//
            ...
            chrome.storage.sync.get(key, function (values) {
                ...
                console.log("got inside the storage function");
                ...
                for (let i = 0; i < highlights.length; i++){
                    ...
                    //insertDiv.innerHTML = highlights[i].highlightTextContent;
...
function openDropdown(dropdown){
    ...
    $(dropdown.querySelectorAll(".dropdownSitesUrl")).each(function (index, element){
        //dropdown.style.height = ((index + 1) * 20) + "px";
        element.style.visibility = "visible";
        //setTimeout(function () { }, 50);
    })

    //dropdown.style.borderBottomStyle = "solid";
...
function closeDropdown(dropdown){
    ...
    //dropdown.style.borderBottomStyle = "hidden";


    dropdown.style.height = "0px";

    $(reversed).each(function (index, element){
        //dropdown.style.height = ((siteLength - index - 1) * 20) + "px";
        element.style.visibility = "hidden";
        //setTimeout(function () { }, 50);
    })
}